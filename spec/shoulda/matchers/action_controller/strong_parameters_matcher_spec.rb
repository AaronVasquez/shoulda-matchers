...
describe Shoulda::Matchers::ActionController::StrongParametersMatcher do
  describe '#description' do
    it 'returns the correct string' do
      ...
      expect(matcher.description).to eq 'permit POST #create to receive parameters :name, :age, and :height'
    end

    context 'when a verb is specified' do
      it 'returns the correct string' do
        options = { action: :some_action, method: :put }
        controller_for_resource_with_strong_parameters(options) do
          params.permit(:name, :age)
        end
        ...
        expect(matcher.description).to eq 'permit PUT #some_action to receive parameters :name'
      end
    end
  end
